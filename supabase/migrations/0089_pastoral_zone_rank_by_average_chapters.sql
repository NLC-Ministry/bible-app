-- Rank pastoral zones by chapters read per active member so differently sized
-- zones are compared fairly. Members with no reading plan/logs remain in the
-- denominator with zero chapters.
...
BEGIN
  ...
  WITH member_progress AS (
    ...
    SELECT
      ...
      ROUND(
        COALESCE(SUM(chapters_read), 0)::NUMERIC / NULLIF(COUNT(*), 0),
        2
      ) AS average_chapters,
...
COMMENT ON FUNCTION public.get_pastoral_zone_leaderboard(UUID, UUID) IS
  'Returns plan-specific pastoral-zone aggregates ranked by average chapters per active member. Zero-read active members remain in the denominator, and chapters are counted across all rounds.';
