...
BEGIN
  ...
  IF actor_profile.id IS NULL
     OR actor_profile.role NOT IN ('admin', 'great_zone_leader', 'zone_leader', 'group_leader') THEN
  ...
  WITH member_details AS (
    ...
    WHERE actor_profile.role = 'admin'
       ...
       OR (
         actor_profile.role = 'group_leader'
         AND EXISTS (
           SELECT 1
           FROM UNNEST(STRING_TO_ARRAY(COALESCE(member.small_group, ''), ',')) AS member_scope(value)
           JOIN UNNEST(STRING_TO_ARRAY(
             COALESCE(NULLIF(actor_profile.managed_groups, ''), actor_profile.small_group, ''), ','
           )) AS actor_scope(value)
             ON BTRIM(member_scope.value) = BTRIM(actor_scope.value)
           WHERE BTRIM(member_scope.value) <> ''
         )
