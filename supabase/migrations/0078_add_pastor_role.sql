-- Migration 0078: Add "pastor" (牧者) role
--
-- New role distinct from senior_pastor (教會牧者): whole-church visibility +
-- full plan-management authority, but no system-management permission
-- (cannot assign roles, manage delegated scopes, or reach ADMIN_RPC_FUNCTIONS
-- / isAdmin()-gated endpoints in nlc-data). Functionally it is a peer of
-- senior_pastor everywhere senior_pastor already has whole-church plan scope,
-- mirrored 1:1 rather than granted anything senior_pastor itself lacks.
--
-- hub_permission_keys/labels are seeded with plausible Member Hub signals
-- ('pastor' / '牧者') per the pattern in migration 0048, but this role is not
-- currently known to be sent by Member Hub — assignment is expected to be
-- manual (role_id) until/unless a real Hub signal is confirmed to match.
--
-- Scope note: this migration intentionally does NOT touch the admin-parity
-- RLS write policies from migrations 0001/0005/0006/0016 (org structure,
-- announcements, global-plan CRUD, user identities) that already let
-- senior_pastor write those tables under RLS. Those only affect the
-- dev/localhost real-Supabase-client login path (production NLC users go
-- through nlc-data, which bypasses RLS entirely), and extending admin-parity
-- DB writes reads as system-management-adjacent — out of scope for "計畫管理
-- 權限，但沒有系統管理權限". Revisit explicitly if pastor ever needs it.
...
INSERT INTO public.role_definitions (
  ...
  hub_permission_keys, hub_permission_labels
) VALUES (
  '10000000-0000-4000-8000-000000000007', 'pastor', '牧者', 25, FALSE,
  TRUE, FALSE, 'church',
  ARRAY['pastor']::TEXT[], ARRAY['牧者']::TEXT[]
)
ON CONFLICT (id) DO UPDATE SET
  ...
  hub_permission_keys = EXCLUDED.hub_permission_keys,
  hub_permission_labels = EXCLUDED.hub_permission_labels;

-- ── can_send_care_reminder (current def: migration 0054) ──
...
AS $can_send_care_reminder$
  SELECT EXISTS (
    ...
    WHERE sender.id = public.current_profile_id()
      ...
      AND (
        public.role_code(sender.role_id) IN ('admin', 'senior_pastor', 'pastor')
        OR (
...
-- ── get_admin_member_team_placements (current def: migration 0071) ──
...
BEGIN
  ...
  IF actor_role NOT IN ('admin', 'senior_pastor', 'pastor', 'great_zone_leader', 'zone_leader', 'group_leader') THEN
  ...
  WHERE candidate.is_active = TRUE
    ...
    AND (
      actor_role IN ('admin', 'senior_pastor', 'pastor')
      OR (
...
-- ── get_reading_team_registration_overview (current def: migration 0074) ──
...
BEGIN
  ...
  IF actor_profile.id IS NULL
     OR actor_role NOT IN ('admin', 'senior_pastor', 'pastor', 'great_zone_leader', 'zone_leader', 'group_leader') THEN
  ...
  scoped_teams AS (
    ...
    WHERE actor_role IN ('admin', 'senior_pastor', 'pastor')
...
-- ── get_joined_plan_members (current def: migration 0073) ──
...
-- ── get_reading_team_statistics (current def: migration 0076) ──
...
BEGIN
  ...
  IF actor_role NOT IN ('admin', 'senior_pastor', 'pastor') THEN
...
CREATE POLICY profiles_select_by_scope ON public.profiles FOR SELECT TO authenticated USING (
  ...
  OR (SELECT my_role FROM public.get_my_profile()) IN ('admin', 'senior_pastor', 'pastor')
  OR (
