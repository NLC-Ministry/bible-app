-- Migration 0066: Fix role_code check in get_reading_team_registration_overview to prevent HTTP 400 Bad Request
...
BEGIN
  ...
  scoped_teams AS (
    ...
    WHERE actor_role IN ('admin', 'senior_pastor')
       OR (actor_role = 'great_zone_leader' AND public.values_overlap(md.pastoral_zone, actor_profile.managed_regions || actor_profile.great_region))
       OR (actor_role = 'zone_leader' AND public.values_overlap(md.pastoral_zone, actor_profile.managed_zones || actor_profile.pastoral_zone))
       OR (actor_role = 'group_leader' AND public.values_overlap(md.pastoral_zone, actor_profile.managed_groups || actor_profile.small_group))
  ),
  plan_aggregates AS (
    SELECT
      gp.id AS plan_id,
