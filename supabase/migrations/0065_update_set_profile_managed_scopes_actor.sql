-- Migration 0065: Allow p_actor_id in set_profile_managed_scopes for Edge Function service_role calls
...
BEGIN
  IF p_actor_id IS NOT NULL AND auth.role() <> 'service_role' THEN
    RAISE EXCEPTION 'p_actor_id can only be specified by service_role';
  END IF;
  ...
  END IF;
  IF target_role <> 'zone_leader' THEN
    normalized_zones := ARRAY[]::TEXT[];
  END IF;
  IF target_role <> 'group_leader' THEN
